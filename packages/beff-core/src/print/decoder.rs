...
impl DecoderFnGenerator<'_> {
    ...
    fn ref_schema_code(&self, schema_ref: &str) -> SchemaCode {
        ...
        let describe_wrapper = Expr::Ident(Ident {
            span: DUMMY_SP,
            sym: "wrap_describe".into(),
            optional: false,
        });

        let describe_wrapped = Expr::Call(CallExpr {
            span: DUMMY_SP,
            callee: Callee::Expr(describe_wrapper.into()),
            args: vec![
                ExprOrSpread {
                    spread: None,
                    expr: describe.into(),
                ...
                ExprOrSpread {
                    ...
                    expr: Expr::Lit(Lit::Str(Str {
                        span: DUMMY_SP,
                        value: schema_ref.to_string().into(),
                        raw: None,
                    }))
                    .into(),
                },
