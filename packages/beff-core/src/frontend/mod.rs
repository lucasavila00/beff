...
impl<'a, R: FileManager> FrontendCtx<'a, R> {
    ...
    fn get_addressed_type(&mut self, addr: &ModuleItemAddress, span: &Span) -> Res<AddressedType> {
        let mut type_walker = TypeWalker { ctx: self };
        type_walker.get_addressed_item(addr, span)
    }
    ...
    fn get_addressed_qualified_type(
        ...
        let mut qualified_type_walker = QualifiedTypeWalker { ctx: self };
        qualified_type_walker.get_addressed_item(addr, span)
    }
