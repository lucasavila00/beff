...
function deepmergeConstructor(options) {
  ...
  const isPrimitiveOrBuiltIn = (
    // @ts-ignore
    typeof Buffer !== "undefined" ? (value) => typeof value !== "object" || value === null || value instanceof RegExp || value instanceof Date || // @ts-ignore
  ...
  function mergeObject(target, source) {
    ...
    let i, il, key;
    for (i = 0, il = targetKeys.length; i < il; ++i) {
      isNotPrototypeKey(key = targetKeys[i]) && sourceKeys.indexOf(key) === -1 && // @ts-ignore
