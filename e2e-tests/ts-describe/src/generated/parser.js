...
const hoisted_ReusesRef_0 = (ctx, input)=>{
    if (ctx.measure) {
        ctx.deps_counter["ReusesRef"] = (ctx.deps_counter["ReusesRef"] || 0) + 1;
        if (ctx.deps["ReusesRef"]) {
            return "ReusesRef";
        }
        ctx.deps["ReusesRef"] = true;
        ctx.deps["ReusesRef"] = describers.ReusesRef(ctx, input);
        return "ReusesRef";
    } else {
        if (ctx.deps_counter["ReusesRef"] > 1) {
            if (!ctx.deps["ReusesRef"]) {
                ctx.deps["ReusesRef"] = true;
                ctx.deps["ReusesRef"] = describers.ReusesRef(ctx, input);
            }
            ...
            return describers.ReusesRef(ctx, input);
        }
    }
};
